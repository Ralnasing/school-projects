...
CEq::CEq(std::shared_ptr<CExpr> first, std::shared_ptr<CExpr> second)
...
CValue CEq::getValue(const std::unordered_map<CPos, CBuilder, CPosHash>& map,
                     ...
                     int                                                 rowMove) const
{
    ...
    // Invalid if any operand is empty or types are mismatched (number vs string)
...
std::shared_ptr<CExpr> CEq::clone() const
{
