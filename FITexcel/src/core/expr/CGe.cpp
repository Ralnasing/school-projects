...
CGe::CGe(std::shared_ptr<CExpr> first, std::shared_ptr<CExpr> second)
...
CValue CGe::getValue(const std::unordered_map<CPos, CBuilder, CPosHash>& map,
                     ...
                     int                                                 rowMove) const
{
    ...
    // Invalid if values are empty or of mismatched types
...
std::shared_ptr<CExpr> CGe::clone() const
{
