...
CGt::CGt(std::shared_ptr<CExpr> first, std::shared_ptr<CExpr> second)
...
CValue CGt::getValue(const std::unordered_map<CPos, CBuilder, CPosHash>& map,
                     ...
                     int                                                 rowMove) const
{
    ...
    // Invalid if either operand is empty or type-mismatched
...
std::shared_ptr<CExpr> CGt::clone() const
{
